...
export async function GET(req: Request, { params }: { params: Promise<{ provider: string }> }) {
  ...
  if (!PROVIDERS.has(provider)) {
    return NextResponse.redirect(new URL("/signin?sso_error=unknown_provider", origin));
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL("/signin?sso_error=start_failed", origin));
  }
}
