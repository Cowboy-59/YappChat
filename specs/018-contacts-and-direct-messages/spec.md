...
### FR-009 — Composer media: emoji, image drag-drop, GIF — **NEW 2026-07-01**
The Chats composer supports rich media, matching the Communities composer:
- **Emoji** — a dependency-free emoji picker (shared `components/chats/EmojiPicker.tsx`, reused by Communities); inserts glyphs at the caret.
- **Images** — drag-drop onto the conversation **or** a 🖼 picker button stages image previews; on send they upload via `POST /api/upload` (existing S3 seam, ≤25 MB) and post as `mediaurl` keys on the existing message endpoint. Live WS messages carry presigned media (engine `sendMessage` already presigns), so images render immediately for both parties.
- **GIF (Giphy)** — a 🎬 picker searches Giphy through a **backend proxy** (`GET /api/gifs/search?q=`) so the `GIPHY_API_KEY` stays server-side (never shipped to the browser). Picking a GIF **re-hosts it to our S3** (`POST /api/gifs/pick {url}` → fetch server-side → `putObject` → returns a key) and sends it as a normal `mediaurl` media message — so GIFs use the same storage/render/replay path as images (no external-URL special-casing in the engine media model, and no client CORS on Giphy's CDN). If `GIPHY_API_KEY` is unset the proxy returns `503 gif_unconfigured` and the picker shows a "GIF search not configured" note. New code: `src/app/api/gifs/search/route.ts`, `src/app/api/gifs/pick/route.ts`, `src/components/chats/GifPicker.tsx`, `src/components/chats/EmojiPicker.tsx`. New env: `GIPHY_API_KEY` (SSM `/yappchat/web/GIPHY_API_KEY` + web task).
