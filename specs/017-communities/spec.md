...
Messaging rides 001/003 on `conversation:{id}` with **membership-checked subscribe**; each message stores **original content + source language** (default = author's account `preferredlanguage`); rendered with author identity from the account profile. Per-message language detection (for an author who writes off their profile language) is an Open Question — see below. **Composer:** the message box is **multi-line** — it **word-wraps and auto-grows** with content (up to a cap, then scrolls). **Enter sends**; **Ctrl+Enter / Shift+Enter inserts a newline**. **Layout:** the composer is **pinned to the bottom**, visually separated from an **independently-scrolling message list** that fills the space above it (the list scrolls, the composer does not move). **Auto-scroll is bottom-anchored:** a new message (own send or inbound) scrolls the list to the newest **only when the viewer is already at the bottom**; if the viewer has scrolled up to read history, an incoming message does **not** yank them down. Opening a space and sending your own message always jump to the bottom.
...
- **Identity**: the assistant is branded **🖖 SPOCK AI** (the system author label shown on its messages). One name across all spaces.
...
- **Trigger — summon by name (not blanket auto-answer)**: SPOCK AI replies **only when a member's message names it** — contains **"Spock"** or **"SpockAI"** (case-insensitive; the trailing "AI"/"AI" and surrounding punctuation are ignored). Plain questions that do **not** name it are left for humans. (This supersedes the earlier "answer anything that looks like a question" heuristic.) The name token is stripped from the text before it is used as the retrieval query.
- **Retrieval — always docs + prior messages**: every summoned answer retrieves over **both** the uploaded/website sources (**pgvector** semantic search) **and** the space's **prior messages** (history), regardless of any per-space history toggle. Answer is **synthesized only from the retrieved sources**, **in the asker's language**, and **cites ≥1 source** (doc title / page anchor / source URL) when grounded.
- **No-answer path**: when nothing relevant is found, SPOCK AI replies with a fixed message — *"I couldn't find this in the docs — I will report this to our support team and someone will help you"* — and does **not** name individual members (FR-007 availability is no longer surfaced in the decline).
- **Model**: latest Claude (default `claude-opus-4-8`) via the spec 002 PA / `aiproviders`.
- **Guardrails**: retrieval is **hard-scoped to this space's sources + this space's history** (never other spaces/communities); bot answers are attributed to a system SPOCK AI author and labeled AI; owner/mod can disable the toggle, edit sources, and trigger re-crawl/re-index; crawl respects `robots.txt` and a page-count cap.
...
**Amendment (2026-07-13):** rebranded to **🖖 SPOCK AI**; trigger changed from blanket question auto-answer to **name-gated summon** ("Spock"/"SpockAI"); retrieval now **always** folds in prior messages alongside the pgvector'd docs; no-answer path reports to the support team instead of naming an available member.
...
## Delta — Implemented 2026-07-13 (SPOCK AI rebrand + name-gated trigger; multi-line composer)

Spec-first (FR-010 + FR-019 amended above before code). Three tweaks to the community space chat.

- **Composer (FR-010)** — the single-line `<input>` in `CommunitiesApp.tsx` became a **word-wrapping, auto-growing `<textarea>`** (caps at ~6 lines, then scrolls). **Enter sends; Ctrl+Enter / Shift+Enter insert a newline.** Height resets after send.
- **Chat layout + bottom-anchored scroll (FR-010, 2026-07-13b)** — the message list and composer are separated so the composer is **pinned to the bottom** and the list **scrolls independently** above it: the communities **page `<main>` wrapper gained `min-h-0`** (the chats page already had it — this was the missing link that let the whole column scroll and carry the composer with it), `CommunitiesApp`'s root is now a bounded `flex flex-1 min-h-0 flex-col` (was `min-h-[70vh]`), the section + message-list `<div>` get `min-h-0`, and the composer is `shrink-0`. Auto-scroll is now **bottom-anchored** via an `atBottom` ref updated on scroll: a new message scrolls to the newest **only when the viewer is already near the bottom**; opening a space and sending your own message force a jump to the bottom. The same bottom-anchored rule was applied to the DM `ChatsApp` (whose layout was already correctly bounded).
- **SPOCK AI rebrand (FR-019)** — the system author label `AI_ASSISTANT_LABEL` is now **"🖖 SPOCK AI"** (was "Assistant"); the client bot label in `CommunitiesApp.tsx` matches. `AI_ASSISTANT_AUTHOR_ID` is unchanged (stable id).
- **Name-gated trigger (FR-019)** — `maybeAutoAnswer` no longer fires on the `looksLikeQuestion` heuristic. It now fires **only when the message names the bot** (`mentionsSpock()` — matches "spock" / "spockai" / "spock ai", case-insensitive). The name token is stripped before the text is used as the retrieval query.
- **Retrieval always includes history (FR-019)** — `retrieve()` now **always** folds the space's prior messages in alongside the pgvector'd doc/website chunks (previously gated behind the `includehistory` config flag).
- **No-answer → support (FR-019)** — `declineText` returns the fixed line *"I couldn't find this in the docs — I will report this to our support team and someone will help you"* and no longer queries/names available members (`availableHelpers` dropped from the decline path).
- Files: `apps/web/src/components/communities/CommunitiesApp.tsx`, `apps/web/src/lib/communities/spaceai.ts`, `apps/web/src/lib/communities/spaceai-answer.ts`. No schema change / no migration.
