...
- **FR-006 — Upload a standalone training video.** The author uploads a video that was never presented live.
  - [ ] The upload is stored like a recording and plays in the replay player identically to a recording reference.
...
- **FR-007 — In-browser document viewing.** Document items (PDF/doc/slides) render inline on the training page; download is optional.
...
- Recording-reference items reuse the existing spec 071 recording routes (`GET /api/presentations/[id]/recording` and `…/recording/share`). Uploaded-video items are served from their own `mediakey` (S3) via a training-media route/signed URL, played through the same `ReplayPlayer` — uploads do **not** create `presentationrecordings` rows.
...
- Document inline-viewing may reuse the space-AI PDF indexing/viewing pipeline already in the repo.
